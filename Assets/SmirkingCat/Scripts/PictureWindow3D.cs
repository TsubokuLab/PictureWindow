...
public class PictureWindow3D : MonoBehaviour
{


    // This
    private static PictureWindow3D _instance;
    public static PictureWindow3D Instance
    {
    ...
    private PictureWindowControl3D _windowController;
    public PictureWindowControl3D WindowController
    {
    ...
    public STEREO_MODE stereo_mode;
    ...
    private void Awake()
    {
        ...
        if(IPD < 0.048f || 0.078f < IPD) IPD = SteamVR.instance.hmd.GetFloatTrackedDeviceProperty(OpenVR.k_unTrackedDeviceIndex_Hmd, ETrackedDeviceProperty.Prop_UserIpdMeters_Float, ref error);
    }
    ...
    private void Update()
    {
        ...
        {
            ResetStereoMode(STEREO_MODE.NONE);
        }

        if (Input.GetKeyDown(KeyCode.Alpha1))
        {
            ResetStereoMode(STEREO_MODE.TOP_AND_BOTTOM);
        }

        if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            ResetStereoMode(STEREO_MODE.SIDE_BY_SIDE);
        }
        ...
        if (WindowCamera == null && _corners.ContainsKey(Corner.TopRight))
        {
            ...
            _windowController.CreateCamera(stereo_mode);
    ...
    public void ResetWindow()
    {
        ...
        if (_windowController) _windowController.DestroyCamera();
    ...
    private void OnDrawGizmos()
    {
        if (_corners.ContainsKey(Corner.TopRight))
        {
            ...
            Gizmos.DrawLine(_corners[Corner.TopLeft], _corners[Corner.TopRight]);
            Gizmos.DrawLine(_corners[Corner.TopLeft], _corners[Corner.BottomLeft]);
            Gizmos.DrawLine(_corners[Corner.BottomRight], _corners[Corner.TopRight]);
            Gizmos.DrawLine(_corners[Corner.BottomRight], _corners[Corner.BottomLeft]);
        }
    }
}
